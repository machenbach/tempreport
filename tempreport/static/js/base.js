...
function getLabel(i) {
	if (mike.tempreport.labels instanceof Array
			&& i < mike.tempreport.labels.length) {
...
function toDateInputString(s) {
	a = s.split("/");
	return a[2] + "-" + pad(a[1])  + "-" + pad(a[0]);
}
...
mike.tempreport.getChartData = function(resp) {
	...
	gapi.client.tempreport.tempreading.getReading({
		'site' : mike.tempreport.site
	}).execute(mike.tempreport.getGaugeData);
}
...
mike.tempreport.showTemps = function() {
	...
	gapi.client.tempreport.tempreading.getLabel({
		'site' : mike.tempreport.site
	}).execute(mike.tempreport.setLabels);

	// get the chart data.  This will kick off the gauge data, then call the method to draw charts
	gapi.client.tempreport.tempreading.readings({
		'site' : mike.tempreport.site,
		'hours' : mike.tempreport.hours,
		'startDate' : document.getElementById('myDate').value
	}).execute(mike.tempreport.getChartData)
}
...
mike.tempreport.drawCharts = function() {
	...
	var gaugechart = new google.visualization.Gauge(document
...
mike.tempreport.enableButtons = function() {
	...
	// document.getElementById('myDate').value = toDateInputString(new Date().toLocaleDateString());
